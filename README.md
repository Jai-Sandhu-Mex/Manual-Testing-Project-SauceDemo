# 🛒 SwagLabs E-Commerce Testing Project

<div align="center">

![Manual Testing](https://img.shields.io/badge/Testing-Manual-blue?style=for-the-badge)
![Status](https://img.shields.io/badge/Status-Completed-success?style=for-the-badge)

**Manual Testing Project for SauceDemo SwagLabs Application**

</div>

---

## 📋 About This Project

This repository contains manual testing documentation for **SwagLabs** e-commerce web application. This project demonstrates professional QA testing practices including test planning, test case design, bug reporting, and requirements traceability.

**Application URL:** https://www.saucedemo.com/

---

## 📁 Test Documentation

All test artifacts are available in Google Docs/Sheets:

| Document | Description | Link |
|----------|-------------|------|
| 📄 **Test Plan** | Testing strategy and approach | [View Document](LINK_TO_TEST_PLAN) |
| 📝 **Test Cases** | Detailed test scenarios with steps | [View Document](LINK_TO_TEST_CASES) |
| 🔗 **RTM** | Requirements Traceability Matrix | [View Document](LINK_TO_RTM) |
| 🐛 **Bug Report** | Defect documentation | [View Document](LINK_TO_BUG_REPORT) |

---

## 🔬 Testing Scope

**Modules Tested:**
- User Login/Logout
- Product Catalog
- Shopping Cart
- Checkout Process
- UI/Navigation

**Browsers Tested:**
- Chrome
- Firefox
- Edge

---

## 📊 Test Summary

| Metric | Count |
|--------|-------|
| Total Test Cases | 75 |
| Passed | 68 |
| Failed | 7 |
| Pass Rate | 90.67% |

**Defects Found:**
- Critical: 1
- High: 2
- Medium: 3
- Low: 1

---

## 🛠 Tools Used

- **Google Docs** - Test Plan
- **Google Sheets** - Test Cases, RTM, Bug Reports
- **Chrome/Firefox/Edge** - Testing Browsers

---

## 💼 Skills Demonstrated

- Test Planning & Strategy
- Test Case Design
- Defect Reporting
- Requirements Traceability
- Documentation

---

## 👤 About Me

**Jai Sandhu**  
4th Year Student | VIT Bhopal  
Aspiring QA Engineer

📧 **Email:** your.email@example.com  
💼 **LinkedIn:** [Your LinkedIn Profile](YOUR_LINKEDIN_URL)

---

<div align="center">

⭐ **Star this repo if you find it helpful!**

*Last Updated: December 2024*

</div>
